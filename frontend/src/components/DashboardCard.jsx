...
function PlaceItem({ item }) {
  ...
  const reviewText = item.review_count === 1 ? '1 review' : `${item.review_count} reviews`
  ...
  return (
    <li>
      <a
        ...
        className="flex items-start gap-3 p-2.5 rounded-lg bg-slate-800/40 hover:bg-slate-800/60 border border-transparent hover:border-slate-700/40 transition-all cursor-pointer group/item"
      >
        {item.image_url ? (
          <img
            src={item.image_url}
        ...
        <div className="min-w-0 flex-1 py-0.5">
          ...
          <div className="flex items-center gap-2 mt-1 text-xs flex-wrap">
            ...
            {item.review_count > 0 && (
              <span className="text-slate-400">{reviewText}</span>
            )}
            {item.price && (
          ...
          {categories && (
            <p className="text-slate-400 text-xs mt-1 truncate">{categories}</p>
          )}
          {item.address && (
            <p className="text-slate-500 text-xs mt-0.5 truncate">{item.address}</p>
          )}
        </div>
